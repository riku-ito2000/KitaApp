<header>
    <nav class="navbar navbar-expand-md" style="height: 70px; background-color: #ffffff;">
        <div class="container d-flex justify-content-center align-items-center" style="max-width: 800px;">
            <a class="navbar-brand d-flex align-items-center" href="{{ url('/') }}" style="background-color: #5a5; color: white; border-radius: 50px; padding: 10px 40px; font-size: 1.5rem;">
                {{ config('app.name', 'Kita') }}
            </a>

            <form class="d-flex ms-3 me-auto" action="{{ route('member.articles.index') }}" method="GET">
            ...
            @auth
                <!-- 記事作成ボタン -->
                <a class="btn btn-sm btn-success ms-2" href="{{ route('articles.create') }}" style="background-color: white; border: 2px solid #5a5; color: black; padding: 8px 30px;">
                    記事を作成する
                </a>
            @endauth
